...
<td class="actions">
<button class="btn btn-secondary" onclick="changePassword(${u.id},'${escapeHtml(u.username)}')">Passwort ändern</button>
...
document.getElementById('userModalTitle').textContent=userId?'Passwort ändern':'Neuer Benutzer';
document.getElementById('userId').value=userId||'';
document.getElementById('username').value='';
document.getElementById('password').value='';
document.getElementById('username').disabled=!!userId;
document.getElementById('passwordLabel').textContent=userId?'Neues Passwort':'Passwort';
...
document.getElementById('userModalTitle').textContent='Passwort ändern';
document.getElementById('userId').value=userId;
document.getElementById('username').value=username;
document.getElementById('username').disabled=true;
document.getElementById('password').value='';
document.getElementById('passwordLabel').textContent='Neues Passwort';
document.getElementById('userModal').classList.add('active');
}
...
if(userId){
const r=await fetch('/api/admin_users.php',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_id:userId,new_password:password})});
const d=await r.json();
showAlert(d.message,d.success?'success':'error');
}else{
const r=await fetch('/api/admin_users.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username,password})});
const d=await r.json();
showAlert(d.message,d.success?'success':'error');
}
closeUserModal();
