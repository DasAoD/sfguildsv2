...
const slotColumns = {
    left: ['Hat', 'BreastPlate', 'Gloves', 'FootWear', 'Belt'],
    right: ['Amulet', 'Ring', 'Talisman', 'Weapon', 'Shield']
};
const gemColors = {
...
function renderCharacterModal(data, fetchedAt, playerName) {
    ...
    const leftEquipHtml = slotColumns.left.map(slot => buildEquipTile(slot, data.equipment?.[slot])).join('');
    const rightEquipHtml = slotColumns.right.map(slot => buildEquipTile(slot, data.equipment?.[slot])).join('');

    const guildNameText = document.getElementById('guildName')?.textContent?.trim() || '';
    const heroHtml = `
        <div class="char-hero">
            <div class="char-equip-col">${leftEquipHtml}</div>
            ...
            <div class="char-equip-col">${rightEquipHtml}</div>
        </div>
    `;
    ...
    const potionsHtml = activePotions.length
        ? activePotions.map(p => `
            <div class="char-potion" title="${escapeHtml(p.typ)} (${escapeHtml(p.size)})">
